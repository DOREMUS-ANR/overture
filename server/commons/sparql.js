...
export default class Sparql {
  ...
  execute(query) {
    ...
    return new Promise((resolve, reject) => {
      this.client.query(query, (err, results) => {
        if (err) reject(err);
  ...
  loadQuery(queryId, opt = {}) {
    if (!queryId) throw Error('The name of the query to load is required');

    if (!opt.lang) opt.lang = 'en;q=0.9, ru-Latn;q=0.2, grm-Latn;q=0.2, el-Latn;q=0.2 *;q=0.1';

    const _file = path.join(queryFolder, `${queryId}.rq`);

    opt.offset = opt.offset || 0;

    const data = this.cache.get(queryId, opt);
    if (data) return data;
